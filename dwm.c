	...
	if (m->lt[m->sellt]->arrange)
		...
		for (c = selmon->clients; c; c = c->next)
			if (ISVISIBLE(c) && c->bw == 0)
				resize(c, c->x, c->y, c->w - 2*m->borderpx, c->h - 2*m->borderpx, m->borderpx, 0);
}
...
manage(Window w, XWindowAttributes *wa)
{
	...
	if(c->isfloating ) {
		...
		c->y = c->mon->my + (c->mon->wh - HEIGHT(c)) / 2;
	}
...
resizemouse(const Arg *arg)
{
	...
	do {
		...
		case MotionNotify:
			...
			if (!selmon->lt[selmon->sellt]->arrange || c->isfloating)
				resizeclient(c, nx, ny, nw, nh, c->bw);
			break;
		}
	} while (ev.type != ButtonRelease);
	XUngrabPointer(dpy, CurrentTime);
	...
	for (c = selmon->clients; c; c = c->next)
	{
		...
		if (c->isfloating || !selmon->lt[selmon->sellt]->arrange)
		{
			if (arg->i != 0 && prev_borderpx + arg->i >= 0)
				resize(c, c->x, c->y, c->w-(arg->i*2), c->h-(arg->i*2), c->bw, 0);
			else if (arg->i != 0)
				resizeclient(c, c->x, c->y, c->w, c->h, selmon->borderpx);
			else if (prev_borderpx > selmon->defaultborderpx)
				resize(c, c->x, c->y, c->w + 2*(prev_borderpx - selmon->defaultborderpx), c->h + 2*(prev_borderpx - selmon->defaultborderpx), selmon->borderpx, 0);
			else if (prev_borderpx < selmon->defaultborderpx)
				resize(c, c->x, c->y, c->w-2*(selmon->defaultborderpx - prev_borderpx), c->h-2*(selmon->defaultborderpx - prev_borderpx), selmon->borderpx, 0);
		}
	}
	arrange(selmon);
}
...
togglefloating(const Arg *arg)
{
	...
	if (selmon->sel->isfloating)
		resize(selmon->sel, selmon->sel->x, selmon->sel->y,
			selmon->sel->w - 2 * (selmon->borderpx - selmon->sel->bw),
			selmon->sel->h - 2 * (selmon->borderpx - selmon->sel->bw),
			selmon->borderpx, 0);
	arrange(selmon);
}
